...
public class LiderController {
    ...
    public String mostrarDashboard(Model model) {
        ...
        Usuario usuarioActual = usuarioRepositorio.findByEmail(correo).orElse(null);

        if (usuarioActual != null) {
            model.addAttribute("usuario", usuarioActual);

            // Añadir estadísticas
            long totalProyectos = proyectoRepositorio.count();
            // Usar el valor correcto de HABILITADO en lugar de ACTIVO
            long proyectosActivos = proyectoRepositorio.countByEstado(EstadoProyecto.HABILITADO);

            // Si el método countByEstado no existe, se puede obtener de manera alternativa
            if (proyectosActivos == 0) {
                List<Proyecto> proyectos = proyectoRepositorio.findAll();
                proyectosActivos = proyectos.stream()
                        .filter(p -> p.getEstado() == EstadoProyecto.HABILITADO)
                        .count();
            }

            // Obtener etapas de proyectos habilitados
            List<Etapa> todasLasEtapas = etapaRepositorio.findAll();
            long etapasActivas = todasLasEtapas.stream()
                    .filter(e -> e.getProyecto() != null && e.getProyecto().getEstado() == EstadoProyecto.HABILITADO)
                    .count();

            // Obtener actividades y contar las no completadas
            List<Actividad> todasLasActividades = actividadRepositorio.findAll();
            long actividadesAsignadas = todasLasActividades.stream()
                    .filter(a -> a.getEstado() != null && a.getEstado() != EstadoActividad.COMPLETADA)
                    .count();

            // Contar actividades por estado para el chart
            long pendientes = todasLasActividades.stream()
                    .filter(a -> a.getEstado() == EstadoActividad.PENDIENTE)
                    .count();
            long enProceso = todasLasActividades.stream()
                    .filter(a -> a.getEstado() == EstadoActividad.EN_PROCESO)
                    .count();
            long completadas = todasLasActividades.stream()
            ...
            model.addAttribute("actividadesAsignadas", actividadesAsignadas);

            // Añadir conteo de actividades por estado para el chart
            model.addAttribute("actividadesPendientes", pendientes);
            model.addAttribute("actividadesEnProceso", enProceso);
            model.addAttribute("actividadesCompletadas", completadas);

            // Añadir proyectos recientes (los últimos 5)
            List<Proyecto> proyectos = proyectoRepositorio.findAll();
            // Ordenar proyectos por fecha de inicio descendente
            proyectos.sort(Comparator.comparing(Proyecto::getFechaInicio).reversed());
            List<Proyecto> proyectosRecientes = proyectos.stream()
                    .limit(5)
